...
namespace agile9.outlook.context.db.Controllers
{
    public class CompaniesController : ApiController
    {
        ...
        public async Task<IHttpActionResult> GetCompany([FromUri] string mail)
        {
            Company company = await db.Companies.Where(x => x.Email.Contains(mail)).FirstOrDefaultAsync();
            if (company == null)
            {
                return NotFound();
            }
